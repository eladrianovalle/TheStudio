...
- Track the spec with its feature: commit it under the specs directory — `.studio/specs/` in a repo that installed Studio, `specs/` in the Studio source repo itself — and link it to the ticket/issue it belongs to, so the design and the work stay tied together.
...
- **Diagnostics:** `stats.py` (pure cross-run aggregation, plus the shipped-features summary read off spec frontmatter — a feature counts as shipped when its spec says `status: shipped`), `obligations.py` (pure; the obligation queue `stats` prints and the session brief names one of — a unit nobody built, a finished spec still at `approved`, evidence past its date), `session.py` (pure; builds the automatic `session.json` health record finalize writes for each run)
...
- Where artifacts land: runs from this repo go to `studio/output/`; runs from a repo that installed Studio go to that repo's `.studio/output/`, and its first run auto-scaffolds `.studio/` and a bridge doc there. Override with `--artifact-root` flag or `STUDIO_ARTIFACT_ROOT` env var. Priority: flag > env > cwd detection. Full resolution chain: `studio/docs/ARCHITECTURE.md`.
