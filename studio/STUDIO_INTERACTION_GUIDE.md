...
2. Follow the loop spelled out inside the file:
   ...
   - **Clarity scores:** As decisions accumulate, per-topic clarity scores track confidence. Topics marked "Settled" (score >= 0.7) become constraints agents won't re-litigate. "Needs work" topics (< 0.4) trigger more decision points. View with `show-clarity`, override with `set-clarity --topic <slug> --score <0.0-1.0>`. Clarity persists across runs in `.studio/clarity.json`.
   - After approval:
