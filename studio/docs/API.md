...
| `check-updates` | The session brief the SessionStart hook runs (you rarely call it by hand). Two independent sources, one JSON object, and silence when neither has news. **An available update:** compares the commit your Studio was installed from against the source's current `origin/main` HEAD; if they differ, it prints a one-line "an update is available" nudge. **Unfinished planned work:** reconciles the Build Plans of your `approved` specs (read from `--target`, never from the working directory) against the `writer:`/`editor:` commits in your log, and names one unbuilt unit with its spec, its title, the count still owed, and the exact `/forge --spec <spec-path> --unit <id>` command — nothing stored, re-derived every session; a unit stops counting when it's built or when a PR adds `- **Dropped:** YYYY-MM-DD — <reason>` under it in the spec. Each source is guarded on its own, so a failure in one can't silence the other. Network is a bounded, best-effort `git fetch` at most once per 24h (cached in `.studio/update-check.json`), so it's near-instant on repeat sessions and safe offline. It nudges once per new upstream commit, never changes files, and always exits 0 (a session-start hook must never fail your session). Opt out durably by creating an empty `.studio/update-check.off` file. Only works on the machine that ran `studio init` (the one with the Studio source); a teammate who merely cloned the repo gets a silent no-op. |
...
The **"Planned work (approved specs vs. git)"** block compares two things already in the repository: the units an `approved` spec's `## Build Plan` planned, and the `writer: <unit_id>` / `editor: <unit_id>` commits git says were built. Nothing is stored — the answer is re-derived every run, so it cannot hold a stale "done". It lists planned units nobody built, names any unit whose writer stopped on purpose (a `writer(stuck):` commit and no build) under its own label, counts the units closed on purpose with a `- **Dropped:** YYYY-MM-DD — <reason>` line in the spec, and gives a rough count of built ids no spec mentions anywhere — rough because units built before `/forge` read a spec's plan land there too. With nothing to report it prints one line saying so. When git cannot be read at all — no work tree, no `git` on PATH, a shallow clone such as CI's `fetch-depth: 1` — the block is not printed at all, because an unknown built set reconciles nothing: reporting no unbuilt units would read as "nothing was ever built", and the one-line "nothing unfinished" says it outright. The reconciliation lives in `stats.py` (`parse_build_plan()`, `built_unit_ids()`, `reconcile_units()`, `format_unit_ledger()`); `--json` does not carry it.
