...
| `stats` | Cross-run diagnostics dashboard: shipped features read from spec frontmatter, verdict/approval rate, decision priority mix + answer rate, session health, planned work still owed against what git says was built, and prepare-usage counts. Supports `--phase`, `--json`, `--artifact-root`. |
