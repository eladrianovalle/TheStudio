...
Read this block honestly: it is a **Studio-source-repo feature**. Across the consuming repos there are 19 specs and every one sits at `approved`, so those repos print the one-line empty state (`No shipped features recorded yet …`) and will keep printing it until someone there ships a spec. With `--json`, the emitted dict gains a `shipped_specs` key holding the same summary (`records`, the `impact` tally, and `recent_changed`).
