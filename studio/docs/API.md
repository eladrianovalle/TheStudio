...
| `stats` | Cross-run diagnostics dashboard: shipped features read from spec frontmatter, verdict/approval rate, decision priority mix + answer rate, session health, planned work still owed against what git says was built (text dashboard only), and prepare-usage counts. Supports `--phase`, `--json`, `--artifact-root`. |
...
| `check-updates` | The session brief the SessionStart hook runs (you rarely call it by hand). Two independent sources, one JSON object, and silence when neither has news. **An available update:** compares the commit your Studio was installed from against the source's current `origin/main` HEAD; if they differ, it prints a one-line "an update is available" nudge. **Something the repository owes:** derives the obligation queue from your `approved` specs (read from `--target`, never from the working directory) and the `writer:`/`editor:` commits in your log, then names exactly one — the most important — with the spec that defines it, the count of everything owed, and either the command that discharges it (`/forge --spec <spec-path> --unit <id>` for a unit nobody built) or, for a spec whose frontmatter or evidence file is what is owed, the edit that does. Nothing is stored and the list is never printed; an obligation stops counting when it is discharged or closed on purpose — a unit with `- **Dropped:** YYYY-MM-DD — <reason>` under it in the spec. Each source is guarded on its own, so a failure in one can't silence the other. Network is a bounded, best-effort `git fetch` at most once per 24h (cached in `.studio/update-check.json`), so it's near-instant on repeat sessions and safe offline. It nudges once per new upstream commit, never changes files, and always exits 0 (a session-start hook must never fail your session). Opt out durably by creating an empty `.studio/update-check.off` file. Only works on the machine that ran `studio init` (the one with the Studio source); a teammate who merely cloned the repo gets a silent no-op. |
| `check-install` | Checks whether the installed Studio is up to date, and shows any files you've edited locally that an `update` would overwrite: both the Studio source and the installed slash commands and workflows. Also guards against a stale *source*: it does a quick, best-effort `git fetch` of the Studio source and, if the source's own checkout is behind its remote (`origin/main`), refuses to report "up to date" (exits non-zero) and compares against `origin/main` instead — so "up to date" can't silently mean "up to date with an out-of-date source." Pass `--no-fetch` to skip the network and compare against already-fetched refs. It also names commands Studio shipped in an older version that are still on disk with nothing recording that Studio wrote them — `update` cannot remove those, so it reports them for you to delete by hand. |
...
| `setup` | Configure Studio for a project: role pack selection, role + phase-persona customization (`.studio/personas.toml`), unstale audit config (`.studio/unstale.toml`), smoke profile (`.studio/smoke.toml`), the `/forge` gate commands (`.studio/implementation_loop.toml`, always written — pre-filled when the stack is recognised, a blank stamped template when it isn't, and never overwritten), cleanup settings. Supports `--target`, `--status`, `--defaults`, `--answers`, `--role-pack`, `--roles`. |
...
The **"What this repository owes (approved specs vs. git)"** block is the obligation queue: everything unfinished, most important first, each entry carrying the command that discharges it or the edit that does. Three kinds appear. A **stale status** is an `approved` spec whose every planned unit is built or dropped and that is not waiting on evidence — the flip to `status: shipped` it owes is what puts a landed feature into the shipped-features block, so it leads the order. **Evidence overdue** is an `approved` spec whose `verification_due` has passed with its `*-eval-results.md` file still blank. **Planned and never built** is a unit an `approved` spec's `## Build Plan` planned that no `writer: <unit_id>` / `editor: <unit_id>` commit mentions, with the `/forge` line that builds it and the `- **Dropped:** YYYY-MM-DD — <reason>` line that closes it on purpose. Under the queue the block keeps the states that are not obligations: a unit whose writer stopped on purpose (a `writer(stuck):` commit and no build), a count of the units closed on purpose, and a rough count of built ids no spec mentions anywhere — rough because units built before `/forge` read a spec's plan land there too. Nothing is stored: the answer is re-derived every run, so it cannot hold a stale "done". With nothing to report it prints one line saying so. When git cannot be read at all — no work tree, no `git` on PATH, a shallow clone such as CI's `fetch-depth: 1` — the block is not printed at all, because an unknown built set reconciles nothing: reporting no unbuilt units would read as "nothing was ever built". The derivation is pure and lives in `obligations.py` (`derive()`, `Obligation`, `SpecView`, `format_queue()`) over `stats.py`'s reconciliation (`parse_build_plan()`, `built_unit_ids()`, `reconcile_units()`, `format_obligations()`); `run_phase._obligations_and_ledger()` is the one place that reads the specs directory and git for it. `--json` does not carry it.
