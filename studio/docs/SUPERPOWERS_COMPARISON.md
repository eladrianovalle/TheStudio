...
| **Enforcement** | Almost entirely prose. No CI workflows exist in the repo; `.pre-commit-config.yaml` lints only the gitignored `evals/` Python | Python. Gates, validators, 992 tests, ruff + pytest in CI |
...
- **They measure prose changes; we argue about them.** `skills/writing-skills/testing-skills-with-subagents.md`
  ...
  variance treated as its own metric. Studio has 992 tests and none of them can tell whether a
...
**Vetting forced.** No new `studio/docs/PROMPT_AUTHORING.md` (that directory is already at 24 files
...
**How it lands.** In `CONTRARIAN_MANDATE` (`studio/scopes.py:41-42`), rewrite the two escape-hatch
...
(`scopes.py:386`, `run_phase.py:962`), carrying the one harvested case where a skip was later shown
