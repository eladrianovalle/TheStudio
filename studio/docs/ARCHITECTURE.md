...
| `stats.py` | Pure cross-run aggregation, formatting, and outcome roll-up (`aggregate_stats`, `format_stats`, `summarize_outcomes`, `summarize_session_health`, `_parse_usage_log`). Also owns `parse_frontmatter`, the single reader of a spec's leading `---` block, shared with `tests/test_spec_verification.py` so the gate and the dashboard can't disagree about what a spec says. Backs the `stats` command; moved out of `run_phase.py` so the number-crunching has no I/O. |
