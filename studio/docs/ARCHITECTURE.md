...
| `config/implementation_loop.toml` | Shipped defaults for the implementation writer/editor loop — `[loop]` and `[editor]` only. There is deliberately no `[gate]` table: the gate commands are detected per repo from its marker files (`impl_loop.STACK_MARKERS` → `resolve_profile`), and a repo Studio can't identify — or one matching two stacks at once — is refused at load with the file and lines to write. A project override at `.studio/implementation_loop.toml` is read *instead of* this file; its `[loop]`/`[editor]` keys fall back to `LoopConfig`'s built-in defaults, not to this file's values, while its `[gate]` keys merge *over* the detected profile. Loaded by `impl_loop.py`. |
