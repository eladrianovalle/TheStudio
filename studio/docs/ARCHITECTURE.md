...
| `findings.py` | Contrarian finding parsing: the machine-readable `FINDING` blockquote format (owns `FINDING_BLOCK_TEMPLATE`, injected into contrarian prompts by `scopes.py`), parsed into `findings.json`, which `finalize` writes at the end of a run. Sibling of `decision_points.py` — a `Finding` mirrors a decision point. |
