...
| `stats.py` | Pure cross-run aggregation, formatting, and the shipped-features roll-up (`aggregate_stats`, `format_stats`, `summarize_shipped_specs`, `summarize_session_health`, `_parse_usage_log`). Also owns `parse_frontmatter`, the single reader of a spec's leading `---` block — `tests/test_spec_verification.py` and `run_phase._shipped_spec_records` both call it, so the gate and the dashboard can't disagree about what a spec says. `build_plan_section` (with `strip_fenced_blocks`) is the matching single reader of a spec's `## Build Plan`: it blanks fenced code blocks first and then takes the *last* Build Plan heading, so a spec that quotes the plan format in a fenced example cannot be mistaken for one that has two plans. Backs the `stats` command; moved out of `run_phase.py` so the number-crunching has no I/O. |
