...
| `findings.py` | Contrarian finding parsing: the machine-readable `FINDING` blockquote format (owns `FINDING_BLOCK_TEMPLATE`, injected into contrarian prompts by `scopes.py`), parsed into `findings.json`, which `finalize` writes at the end of a run. Sibling of `decision_points.py` — a `Finding` mirrors a decision point. `findings.json` carries a `schema_version`; the loader reads that shape and the unversioned bare list every pre-2026-09 run wrote, so no archived run is orphaned. |
| `verifier.py` | Independent finding-verifier core: selects Medium-confidence findings, applies a verdict, and writes the adjusted confidence back into `findings.json`. Pure, gate-tested logic behind the `.claude/workflows/finding-verifier.js` Workflow, whose fresh agent re-checks each finding from ONLY its quote — never the contrarian's reasoning (the anti-anchoring firewall). **`finalize` names the step**: when a run leaves Medium-confidence findings with no verdict, it prints the `/finding-verifier <run_dir>` invocation. Studio runs no agents itself, so without that line the step lived only in this doc and was never taken — every finding in every repo carried a null verified confidence. |
...
| `cleanup.py` | TTL-based (30 days) and budget-based (900 MB) cleanup of run artifacts, plus loose file removal for legacy artifacts outside run directories. **A finalized run — any run `finalize` has been through, whatever status it recorded — is never deleted, by either rule.** Only unfinished prepares age out; when the budget cannot be met without touching a finalized run, cleanup reports how far over it is and leaves them alone. |
...
| `stats.py` | Pure cross-run aggregation, formatting, and the shipped-features roll-up (`aggregate_stats`, `format_stats`, `summarize_shipped_specs`, `summarize_session_health`, `_parse_usage_log`). Also owns `parse_frontmatter`, the single reader of a spec's leading `---` block — `tests/test_spec_verification.py` and `run_phase._shipped_spec_records` both call it, so the gate and the dashboard can't disagree about what a spec says. `build_plan_section` (with `strip_fenced_blocks`) is the matching single reader of a spec's `## Build Plan`: it blanks fenced code blocks first and then takes the *last* heading that reads exactly `## Build Plan`, so neither a spec that quotes the plan format in a fenced example nor one keeping an earlier plan under a labelled heading (`## Build Plan (as originally proposed)`) can be read as the plan in force. `near_miss_build_plan_headings` covers the other side of that exactness: when the only Build Plan heading a spec has is one the exact match misses — a label, a lower-case word, a third `#`, a doubled space, up to three spaces of indentation — the plan under it would otherwise be invisible to every reader, so rule 7 names the heading and asks for the exact one. Three spaces is markdown's own bound, so an indented heading a reader sees rendered normally is refused rather than passed over; at four it is an indented code block, and a spec quoting the format in its notes stays quiet. `indistinguishable_build_plan_headings` is the one case where a second Build Plan heading is a defect even with the real one present: a label says the section is not the plan, but a doubled space, a lower-case word or a third `#` says nothing of the kind, so rule 7 names it before it reads any units. Saying nothing is the whole test, which is why the heading level is not part of it — `### Build Plan` beside the real one otherwise falls through both nets and its units are read by nothing. Both nets stop at level 2, though: a spec's frontmatter is YAML that carries its notes on `# ...` comment lines and nothing here strips it, so reading a single `#` as a heading would refuse a spec over a line no renderer shows. `near_miss_build_plan_section` reads a near-miss plan anyway for the id-collision check, which has to see the ids of `draft` and `shipped` specs that rule 7 never complains about; it is bounded by its heading's own depth, with a level-3 unit heading at that depth continuing the plan, so a `### Build Plan` does not swallow the sibling `###` sections after it. That continuation is level 3 only — the one depth a unit is written at — because at every depth it defeats the bound for the real heading too, where a ``## `stats.py` — what changes`` after the plan would read as more plan. The completion ledger reads the same section: `parse_build_plan` accepts any entry opener carrying a backticked `snake_case` id — the heading form rule 7 pins and the numbered-bold form most older specs use — because rule 7 runs in Studio's CI and nowhere else, so a reader that demanded its shape would find nothing in almost every consuming repo's specs. `built_unit_ids` reads `writer:` / `editor:` ids out of a commit log and keeps `writer(stuck):` escalations in a separate set (built wins when a unit has both), `reconcile_units` sorts planned units into unbuilt, escalated, dropped and built, and `format_obligations` renders that block from the obligation queue plus what the ledger knows that the queue has no opinion about. `DROPPED_LINE` — the date-and-reason line that closes a unit without building it — is shared with rule 7 so the shape the suite demands and the shape the ledger reads cannot drift apart. Backs the `stats` command; moved out of `run_phase.py` so the number-crunching has no I/O. |
| `obligations.py` | What this repository owes, derived and never stored: `derive()` turns approved specs (`SpecView`) plus a reconciled `UnitLedger` into a sorted list of `Obligation`s, and `format_queue` renders them. Three kinds — a unit nobody built, a spec whose work is finished while its frontmatter still says `approved`, and a spec past the date it promised evidence by. Pure in the strict sense: no filesystem, no git, and no clock of its own — the caller passes `today`, which is what makes a deadline testable. It returns an empty list whenever `ledger.built_known` is False, inheriting the ledger's own rule that silence beats a lie. `run_phase._obligations_and_ledger` is the one place that reads the specs directory and git for it; `show_stats` prints the whole queue and `_unfinished_context` names the first one in the session brief. |
...
| `config/implementation_loop.toml` | Shipped defaults for the implementation writer/editor loop — `[loop]` and `[editor]` only. There is deliberately no `[gate]` table: the gate commands come from the repo's own `.studio/implementation_loop.toml` and from nowhere else, so a repo that has written no file — or written one with a blank `test_command` — is refused at load, with the refusal naming that file and saying whether the key was blank or the file missing. A project file is read *instead of* this one; its `[loop]`/`[editor]` keys fall back to `LoopConfig`'s built-in defaults, not to this file's values, while a `[gate]` key it leaves out is empty rather than defaulted. Detection (`impl_loop.STACK_MARKERS` → `resolve_profile`) survives as `/studio-setup`'s opening guess when it writes that file; the loader never calls it. Loaded by `impl_loop.py`. |
