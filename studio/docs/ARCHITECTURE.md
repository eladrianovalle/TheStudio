...
| `findings.py` | Contrarian finding parsing: the machine-readable `FINDING` blockquote format (owns `FINDING_BLOCK_TEMPLATE`, injected into contrarian prompts by `scopes.py`), parsed into `findings.json`. Sibling of `decision_points.py` — a `Finding` mirrors a decision point. |
...
| `stats.py` | Pure cross-run aggregation, formatting, and outcome roll-up (`aggregate_stats`, `format_stats`, `summarize_outcomes`, `summarize_session_health`, `detect_trend_alerts`, `_summarize_metrics`, `_parse_usage_log`). Backs the `stats` command; moved out of `run_phase.py` so the number-crunching has no I/O. `detect_trend_alerts` adds a delta-based "Trend Alerts" block — run-over-run regressions (rating falling, tokens/cost rising) that persist across 2+ consecutive runs, so a single blip never fires. |
...
| `setup.py` | Setup wizard: project configuration after install. Tracks setup state in `.studio/SETUP.json` and authors the per-repo config files with incremental versioned steps: role overrides, phase personas (`.studio/personas.toml`), scopes, cleanup, the `/unstale` audit override (`.studio/unstale.toml`), and the `/smoke` profile (`.studio/smoke.toml`). |
