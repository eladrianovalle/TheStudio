...
- Track the spec with its feature: commit it under the specs directory — `specs/` in this repo, `.studio/specs/` in a repo that installed Studio — and link it to the ticket/issue it belongs to, so the design and the work stay tied together.
