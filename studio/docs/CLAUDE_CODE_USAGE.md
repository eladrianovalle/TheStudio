...
**Config**: `implementation_loop.toml` (override at `.studio/implementation_loop.toml`): `mandate = "off"` disables the editor pass; `read_scope`, `output_budget`, `require_mutation_check`, `static_checks`, and `test_command` tune the loop. Knobs reach the workflow by running the module as a script (`python .studio/source/impl_loop.py`, or `python studio/impl_loop.py` in the Studio source repo).
