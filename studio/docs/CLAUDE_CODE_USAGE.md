...
**The gate commands come from your repository, not from a shipped default.** Studio ships no `[gate]` table. It looks at the marker files at your repo root and uses that stack's commands — `pytest -q` + `ruff check {paths}` + `mutmut run` for a Python project, `npm test` + `npm run lint` for a Node one that declares those scripts. When it can't identify the repo, or finds two stacks at once (a Rust game whose `package.json` is only CI tooling), it **refuses to start** and prints the exact file and three lines to write, instead of running a command that was never going to work here and failing your unit for a reason that has nothing to do with your code. Set `[gate] test_command` — plus `static_checks`, `require_mutation_check`, `mutation_command` if you need them — in `.studio/implementation_loop.toml`. `static_checks` holds **commands**, not tool names: every one of them runs and they all have to pass. Write `{paths}` where you want the paths the unit is scoped to substituted in (`"ruff check {paths}"`), and leave the token out when the command already names its own target (`"make lint"`, `"npm run lint"`) — that one runs exactly as written. A leftover bare name like `"ruff"` is refused at load, with the line to write instead, rather than quietly running nothing. What you set there merges *over* what was detected, so a Unity repo naming only its test wrapper doesn't inherit another language's linter. `/studio-setup` always leaves that file behind, so the commands are somewhere you can read and edit them: filled in with whatever it detected when it recognises your stack, and blank when it doesn't — every key present, with the sentence saying what it found and why it's no help, and a marker on the one line you have to write. It signs the first line so a later run can tell its own untouched template from your file, and it never overwrites one you already wrote.
