...
**The gate commands come from your repository, not from a shipped default.** Studio ships no `[gate]` table. It looks at the marker files at your repo root and uses that stack's commands — `pytest -q` + `ruff` + `mutmut run` for a Python project, `npm test` for a Node one that declares a `test` script. When it can't identify the repo, or finds two stacks at once (a Rust game whose `package.json` is only CI tooling), it **refuses to start** and prints the exact file and three lines to write, instead of running a command that was never going to work here and failing your unit for a reason that has nothing to do with your code. Set `[gate] test_command` — plus `static_checks`, `require_mutation_check`, `mutation_command` if you need them — in `.studio/implementation_loop.toml`. What you set there merges *over* what was detected, so a Unity repo naming only its test wrapper doesn't inherit another language's linter. `/studio-setup` writes that file for you with whatever it detected, so the commands are somewhere you can read and edit them — and it never overwrites one you already wrote.
