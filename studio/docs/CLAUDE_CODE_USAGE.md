...
### The session brief (automatic, at session start)

You don't have to remember to check whether your installed Studio is current, or to go looking for
work an approved spec planned and nobody built. When you run `studio init` (or `update`), Studio
installs a small **SessionStart hook** into your project's `.claude/settings.local.json`. At the
start of each Claude Code session it runs `check-updates`, which has two things it might tell you
— and says nothing at all when it has neither.
...
- **Quiet and cheap.** The update half checks the network at most once a day (bounded, best-effort
  ...
  unfinished-work half is a single `git log` with a five-second ceiling. It works offline, never
  changes your files, and can never slow down or break a session — if anything goes wrong it just
...
**Building in a git worktree (`--work-dir <path>`)**: the loop's git commands are text inside agent prompts, so by default they run wherever the agent's shell starts — the checkout you invoked `/forge` from, on whatever branch it has open. Point `--work-dir` at a worktree of this repo and both agents are told to `cd` there first and keep everything inside it (tests, static check, git), with their git commands rendered as `git -C "<path>"`. The `cd` is the real mechanism, not the pinned git: the test command and the static check aren't git commands, so pinning git alone would let a unit commit to the right branch and then report green tests from a different tree. **The path is validated before the loop starts** — `/forge` checks it before spawning any agent and stops the run with one named reason: `missing`, `not-a-worktree`, `different-repo` (a worktree of some other repository), or `unquotable` (a path holding a character outside the accepted set — letters, digits, and `/ . _ - ~` or a space). The accepted set is deliberately narrow: it keeps a character nobody anticipated out of the `git -C "<path>"` quoting, and the price is that a legitimate path holding a `+`, a `(` or an accent gets refused too. So a typo costs you nothing instead of failing an hour into a run. **Honest limit:** this reduces the blast radius of a run pointed at the wrong directory; it cannot make that impossible, because every one of those instructions is prompt text an agent may ignore and the check happens once, at the start. Without the flag, nothing changes.

**Config**: `implementation_loop.toml` (override at `.studio/implementation_loop.toml`): `mandate = "off"` disables the editor pass; `read_scope` and `output_budget` tune the editor. Knobs reach the workflow by running the module as a script (`python .studio/source/impl_loop.py`, or `python studio/impl_loop.py` in the Studio source repo).

**The gate commands come from your repository's own file, and from nowhere else.** Studio ships no `[gate]` table and works nothing out at run time: `.studio/implementation_loop.toml` is the one thing that says how `/forge` gates this repo, and a `[gate]` key you leave out of it is empty — never another language's default. If that file isn't there, or its `test_command` is blank, `/forge` **refuses to start** and says which of the two it is, names the file, and gives the exact lines to write — instead of running a command that was never going to work here and failing your unit for a reason that has nothing to do with your code. Set `[gate] test_command` — plus `static_checks`, `require_mutation_check`, `mutation_command` if you need them — in that file. `static_checks` holds **commands**, not tool names: every one of them runs and they all have to pass. Write `{paths}` where you want the paths the unit is scoped to substituted in (`"ruff check {paths}"`), and leave the token out when the command already names its own target (`"make lint"`, `"npm run lint"`) — that one runs exactly as written. A leftover bare name like `"ruff"` is refused at load, with the line to write instead, rather than quietly running nothing. `/studio-setup` always leaves that file behind, so the commands are somewhere you can read and edit them: filled in from your repo's marker files when it recognises the stack — `pytest -q` + `ruff check {paths}` for a Python project, `npm test` + `npm run lint` for a Node one that declares those scripts — and blank when it doesn't, every key present, with the sentence saying what it found and why it's no help, and a marker on the one line you have to write. The mutation gate is the one thing it will not assume for you: `mutmut run` is written either way, but `require_mutation_check` is turned on only if your repo sets mutmut's `paths_to_mutate` — without that, `mutmut run` goes looking for a `src/` directory you may not have and fails your unit over its own configuration. Everywhere else the gate is off and the file tells you what to add to switch it on. That guess happens once, at setup, where a wrong one costs you an edit rather than a mis-gated build. It signs the first line so a later run can tell its own untouched template from your file, and it never overwrites one you already wrote.
...
`/forge` and `/spec` carry the same rule. If the repo has a code index or symbol search, the agent uses it to **locate** code before grepping or opening files blind. But an index entry is a summary some tool wrote, and it can lag the file it describes — so the index only says *where*. The agent opens the real file at the line it was handed and quotes what it reads there. An index summary or an inlined source excerpt is never accepted as the quote; a quote taken from a summary counts as `(unverified)`, which the contrarian's own confidence rules already push to the bottom and drop unless it would be fatal.

Studio names no particular tool and installs nothing. Your repo supplies the index and names the command in its own `CLAUDE.md`. A repo without one behaves exactly as it did before — the instruction has nothing to point at.
...
| Command | Purpose |
|---------|---------|
| `/spec` | Maps a feature's architecture into an approved, source-of-truth spec before you build it. Runs a discovery-forward, single advocate/contrarian `tech`-phase pass — scope-paced as an alignment scope (Open-Questions Pre-Flight surfaces and answers the architectural unknowns) then a depth scope (pressure-tests and finalizes the design) — then synthesizes one document that explains the feature in plain language *and* build-ready technical detail with a Mermaid diagram. The run **ends by pushing the spec as a pull request** at `status: draft` — that PR is the review, merging it is the approval, and the status flip is a follow-up commit so a document never claims approval in the change that requests it. The PR body carries what the debate *rejected* and why, which is the part that keeps a settled question from reopening later. Once merged it lives (tracked) under `specs/` (or `.studio/specs/` in a consuming repo), linked to its ticket. Its **Build Plan is a contract, not a summary**: every unit gets a snake_case `unit_id` and three-to-six checkable acceptance criteria, which is what `/forge --spec <slug> --unit <id>` reads and grades the built unit against. If the feature is **prompt-shaped** — its behavior lives in an agent's prompt, where no failing test can tell you it broke — the spec also carries a `## Verification` section naming the pass criterion up front, and approval creates a `<slug>-eval-results.md` sibling for the evidence. Until that file is filled in, the spec stays `approved` and nobody may call the feature working; flipping it to `shipped` with placeholders left in — or with one of that file's headings dropped or left unanswered — fails CI. Approval also writes a `verification_due` date 30 days out, and once that date passes with the evidence still blank CI fails too — until you record what you found or move the date. Args: feature text, `--ticket`, `--id`, `--max-iterations` (default 2), `--plan`. |
...
| `/studio-setup` | Configure project's Studio installation: role packs, role/phase-persona customization (`.studio/personas.toml`), unstale audit config (`.studio/unstale.toml`), smoke test config (`.studio/smoke.toml`), the `/forge` gate commands (`.studio/implementation_loop.toml`), cleanup settings. |
