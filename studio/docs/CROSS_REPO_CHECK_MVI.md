...
`check-install` compares against the Studio source repo on the local filesystem, and by default runs a `git fetch` in that source first so it can also tell you the source itself is behind its upstream. Pass `--no-fetch` to skip that and stay entirely offline — the original zero-dependency behaviour. When the source can't be reached (the recorded path is missing, moved, or points back at the installed snapshot), the command prints a `WARNING:` that says it cannot compare against live source and points you at the re-run command, so the scope of the comparison is never silently misrepresented.
