...
### Added
- **Quality ratings & cross-run stats** — the diagnostics + fine-tuning feedback loop. New `rate` subcommand records a human 1-5 quality score + optional note per run into `rating.json` (the human counterpart to the agent `verdict`). New `stats` subcommand is the first cross-run view: it reads every run's `run.json`/`rating.json`/`decisions.json` plus the previously write-only `.studio/usage.log` and prints a dashboard — verdict/approval rate, rating count/avg/by-phase + lowest-rated improvement targets, token/cost efficiency, decision priority mix + answer rate, prepare-usage counts. `--phase` filter, `--json` output. Pure `aggregate_stats()` / `format_stats()` split.
- **Auto rate-prompt at end of run** — closes the loop without relying on memory. `finalize` ends with a rating prompt: interactive (`Rate this run 1-5 (Enter to skip)`) when attached to a TTY, a copy-paste `rate` nudge otherwise (never blocks stdin in automation). New `finalize --no-rate-prompt` flag. The `/run-phase` and `/run-studio-phase` slash commands pass `--no-rate-prompt` and add a closing "Rate this run" step so the assistant asks the user conversationally (the real human touchpoint). 15 new tests total for the feature.
...
- 14 top-level Python modules in `studio/` plus the new `integrations/` subpackage (`slack_digest.py`); 577 tests.
...
- 483 tests (up from 372)
- 13 Python modules in studio/ (plus validators/)
