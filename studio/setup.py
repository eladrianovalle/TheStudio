...
def _format_personas_toml(customizations: Dict[str, Dict]) -> str:
    ...
    def _q(value: str) -> str:
        # Escape for a TOML basic string: backslash and quote first, then the
        # control chars TOML forbids as literals (newline/tab/CR/backspace/
        # form-feed) — otherwise a multi-line persona value writes a file
        # that tomllib refuses to parse on the next run.
    ...
    for phase, fields in customizations.items():
        ...
        if isinstance(implementer, dict):
            ...
            if "title" in implementer:
                lines.append(f"title = {_q(implementer['title'])}")
            if "deliverables" in implementer:
                items = ", ".join(_q(d) for d in implementer["deliverables"])
