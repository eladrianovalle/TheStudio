...
2. **Spot mutation check** — pick 2-3 of the most critical assertions you fixed or flagged. Deliberately break the production code they guard (change `+` to `-`, flip a condition, return early). Run the relevant tests. If they still pass, the fix didn't work — redo it. Revert the mutations after checking.
